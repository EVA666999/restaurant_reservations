...
class CreateReservation(BaseModel):
    customer_name: str = Field(..., min_length=1, max_length=50, description="Имя клиента")
    table_id: int = Field(..., gt=0, description="Id стола")
    reservation_time: datetime = Field(
        default=None,
        description="Время бронирования (МСК)"
    )
    duration_minutes: int = Field(..., gt=15, le=MAX_DURATION_MINUTES, description="Длительность бронирования в минутах")
