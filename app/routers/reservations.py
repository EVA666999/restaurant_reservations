...
async def create_reservations(
    ...
    reservation_time = create_reservation.reservation_time.astimezone(MSK_TIME).replace(tzinfo=None)
    new_reservation_end_time = reservation_time + timedelta(minutes=create_reservation.duration_minutes)
    
    existing_reservations_query = select(Reservations).where(
        Reservations.table_id == create_reservation.table_id
    ...
    for existing_reservation in existing_reservations:
        existing_end_time = existing_reservation.reservation_time + timedelta(minutes=existing_reservation.duration_minutes)
        
        if (reservation_time < existing_end_time and 
            new_reservation_end_time > existing_reservation.reservation_time):
...
async def delete_table(db: Annotated[AsyncSession, Depends(get_db)], reservation_id: int):
        reservation = await db.scalar(select(Reservations).where(Reservations.id == reservation_id))
