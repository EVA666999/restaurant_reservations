...
async def get_all_tables(db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if not tables:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="There is no table found"
        )
    return tables


@router.post("/", status_code=status.HTTP_201_CREATED, summary="Создание стола")
async def create_table(
    db: Annotated[AsyncSession, Depends(get_db)], create_table: CreateTable
):
    """Создает новый стол"""
    new_table = Tables(
